...
>
> `width`/`height`는 캔버스 드로잉 버퍼와 래퍼 표시 크기를 지정합니다. React/Vue/Svelte 래퍼에서 `responsive` prop을 켜면 부모 컨테이너가 좁아질 때 비율을 유지하며 자동 축소됩니다. 자세한 내용은 [반응형 크기 조정](/kr/docs/configuration#반응형-크기-조정) 참고.
